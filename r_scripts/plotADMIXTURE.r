...
spaces<-0
for(i in rep){spaces=c(spaces,rep(0,rep[i]-1),0.5)}
...
png(file=paste0(opt$outPrefix,".png"),width = 2000, height = 1200,res=200)
 par(mfrow=c(maxK-1,1),mar=c(0,1,0,0),oma=c(2,1,9,1),mgp=c(0,0.2,0),xaxs="i",cex.lab=1.2,cex.axis=0.8)
 # Plot minK
 bp<-barplot(t(as.matrix(tbl[[1]][order(labels$n),])), col=topo.colors(n=minK),xaxt="n", border=NA,ylab=paste0("K=",minK),yaxt="n",space=spaces+1)
 axis(3,at=bp,labels=labels$ind[order(labels$n)],las=2,tick=F,cex=0.6)
 # Plot higher K values
 if(maxK>minK)lapply(2:(maxK-1), function(x) barplot(t(as.matrix(tbl[[x]][order(labels$n),])), col=topo.colors(n=x+1),xaxt="n", border=NA,ylab=paste0("K=",x+1),yaxt="n",space=spaces+1))
