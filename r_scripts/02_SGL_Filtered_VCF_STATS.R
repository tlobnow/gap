################################################################################
# SINGLE SAMPLE FILTERED VCF STATISTICS ########################################
################################################################################
...
INDS <- read.delim("/SAN/Ctyzzeri/gap/scripts/lists/vcf_inds", header = F)
INDS <- INDS$V1
...
for (i in INDS) {
  ...
  # prepare plots
  var_qual_plot  <- ggplot(var_qual,  aes(qual)) + geom_density(fill = "red", colour = "black", alpha = 0.3) + theme_light() + ylim(0,0.0005)
  ...
  # save as png
  png(paste0("/SAN/Ctyzzeri/gap/results/figures/stats_fixedYaxis/", i, ".filtered.stats.png"))
  plot(stats)
