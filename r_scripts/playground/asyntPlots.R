...
alignment <- import.paf(paste0("/SAN/Ctyzzeri/gap/results/paf/", IND,".paf.gz"))
...
alignments <- subset(alignment, Rlen >= 100 & query=="1" & reference == "Ctyz_1")
...
#plot.alignments(alignments, sigmoid=TRUE, Rfirst=1, Rlast = 100, Qfirst=1, Qlast=100)
...
ref_data <- import.genome(fai_file=("/SAN/Ctyzzeri/gap/resources/CryptoDB-57_CtyzzeriUGA55_Genome.fasta.fai"))
query_data <- import.genome(fai_file=paste0("/SAN/Ctyzzeri/gap/results/fasta/", IND, ".fasta.fai"))
...
reference_scafs <- c("Ctyz_1", "Ctyz_2", "Ctyz_3", "Ctyz_4", "Ctyz_5", "Ctyz_6", "Ctyz_7", "Ctyz_8")
query_scafs <- c("1", "2", "3", "4", "5", "6", "7", "8")

#keep only alignments involving these scaffolds
alignments <- subset(alignment, query %in% query_scafs & reference %in% reference_scafs)
...
synblocks <- get.synteny.blocks.multi(alignments, min_subblock_size=100)
...
alignments_5k <- subset(alignment, Rlen>= 5000 & Qlen>= 5000)
